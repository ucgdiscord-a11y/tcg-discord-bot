...
class ConsentView(discord.ui.View):
    ...
    @discord.ui.button(label="✅ 同意して全機能を解放", style=discord.ButtonStyle.green, custom_id="consent_v4")
    async def agree(self, interaction: discord.Interaction, button: discord.ui.Button):
        role = interaction.guild.get_role(ROLE_ID)
        try:
            await interaction.user.add_roles(role)
            await interaction.response.send_message("承認されました！", ephemeral=True)
        except: await interaction.response.send_message("エラー：権限不足です。", ephemeral=True)

class RegistrationView(discord.ui.View):
    def __init__(self): super().__init__(timeout=None)
    @discord.ui.button(label="📝 TCG IDを登録する", style=discord.ButtonStyle.primary, custom_id="reg_v4")
...
class RegionButtons(discord.ui.View):
    ...
    async def update_role(self, interaction: discord.Interaction, role_name: str):
        ...
        if new_role:
            ...
            await interaction.response.send_message(f"✅ 「{role_name}」に設定しました！", ephemeral=True)
...
class MyBot(commands.Bot):
    ...
    async def check_twitter(self):
        # ログ出力（RenderのLogsで見えるようになります）
        now = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        print(f"[{now}] Twitterチェック中... (URL: {RSS_URL})")

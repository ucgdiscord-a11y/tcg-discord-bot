...
ANNOUNCE_CH_ID = 1476095569595334718 # 速報・誘導用
# ===========================================
...
class RulesOnlyView(discord.ui.View):
    ...
    @discord.ui.button(label="✅ 同意して全機能を解放", style=discord.ButtonStyle.green, custom_id="v18_agree")
...
class RegistrationOnlyView(discord.ui.View):
    def __init__(self): super().__init__(timeout=None)
    @discord.ui.button(label="📝 TCG IDを登録する", style=discord.ButtonStyle.primary, custom_id="v18_reg")
...
class PointsOnlyView(discord.ui.View):
    def __init__(self): super().__init__(timeout=None)
    @discord.ui.button(label="🏆 累計ポイントを確認する", style=discord.ButtonStyle.primary, custom_id="v18_pts")
...
class RegionButtonsView(discord.ui.View):
    ...
    @discord.ui.button(label="関東", style=discord.ButtonStyle.primary, custom_id="v18_r2")
    async def b2(self, it, b): await self.update_role(it, "関東")
    # 他の地域のボタンはsetup_allで呼び出す形式にしてコードを軽量化しています
...
class MyBot(commands.Bot):
    ...
    async def on_ready(self):
        ...
        print(f'✅ ログイン成功: {self.user.name} (V18 Final)')
...
if __name__ == "__main__":
    ...
    if TOKEN:
        try: bot.run(TOKEN)
        except Exception as e: print(f"❌ ログインエラー: {e}")
